/**
 * Allen Atlas Oblique Slice Planner — Stage 6
 * Extends Stage 5 with:
 *   A. Euler axis remapping (FIX 1): LEFT slider (TILT) → Euler X (somersault); right-panel
 *      numeric input (ROLL) → Euler Z (axial spin). Swap: buildSlicePlane, animation-loop
 *      plane widget rotation, ghost plane rotation, TC preset angle.
 *   B. Camera scale fix (FIX 2): initial radius 1600, far plane 50000, zoom clamp
 *      300–6000, locked view position corrected to (800,-300,1200), PlaneGeometry
 *      enlarged to 1500×1200, brain shell scale updated to makeScale(570,400,650).
 *   C. CCF_STRUCTURES_RAW positions updated (FIX 3): refined CCF coordinates, enlarged
 *      radii (~20% increase for better visibility at 1600 camera distance).
 *
 * Stage 6 preserves ALL Stage 5 logic exactly:
 ...
 *   - viewLocked camera mode (Stage 5)
 *   - SavedSlices: save, recall, delete, thumbnail, export/import session JSON (Stage 5)
 *
 ...
 *
 * SYSTEM_SPEC.md v3 §Stage 6 Addendum
 * INTERFACE_CONTRACTS.md v3 §Stage 6 Addendum
 */
...
// ---------------------------------------------------------------------------
// Stage 6 FIX 3: Refined CCF coordinates (cortex pushed more anterior, thalamus
// slightly adjusted) and radii scaled ~20% for better visibility at camera radius 1600.
// VPL id:718 unchanged (corrected in Stage 4).
...
//
// Stage 6 FIX 1: Euler argument order swapped so that:
//   roll  → Euler X = somersault (blade tips forward/back) — driven by LEFT frame slider
//   yaw   → Euler Y = left-right swing                     — driven by BOTTOM frame slider
//   pitch → Euler Z = axial spin (head-tilt)               — driven by right-panel numeric input
//
// Args:
//   pitch:      rotation about Z axis (axial spin), degrees [-90, 90]
//   yaw:        rotation about Y axis (left-right swing), degrees [-90, 90]
//   roll:       rotation about X axis (somersault/tilt), degrees [-90, 90]
...
function buildSlicePlane(pitch, yaw, roll, apMicrons, lrMicrons = 0, dvMicrons = 0) {
  const THREE = window.THREE;
  // Stage 6 FIX 1: roll → Euler X (somersault), yaw → Euler Y (swing), pitch → Euler Z (axial spin).
  // Previously: pitch→X, yaw→Y, roll→Z (wrong: LEFT slider produced axial spin instead of somersault).
...
// Preset Engine
// ---------------------------------------------------------------------------
// Stage 6 FIX 1: TC preset updated — the -35° somersault tilt now lives in
// sliceState.roll (which maps to Euler X after the axis remapping). Previously
// it was in sliceState.pitch (Euler X), which is now the axial-spin axis (Euler Z).
// Hippocampal preset: yaw still drives Euler Y — no change needed.
...
export default function AllenAtlasSlicePlanner() {
  ...
  // Stage 6 FIX 2: initial radius 800 → 1600 (brain shell 760 units tall; 800 clipped viewport).
  ...
  useEffect(() => {
    ...
    // Stage 6 FIX 2: updated from (300,-150,500) (~602 units — closer than initial radius,
    // clipped 34% of brain height) to (800,-300,1200) (~1485 units — comfortable 3/4 view).
    lerpTargetPos.current = new THREE.Vector3(800, -300, 1200);
    ...
    // --- Camera ---
    // Stage 6 FIX 2: far plane 10000 → 50000 (adequate margin once real PLY meshes load).
    ...
    // Stage 6 FIX 2: scale updated (580,380,480) → (570,400,650).
    // Gives shell 1140×800×1300 Three.js units — approximates real CCF bounding box
    // at ellipsoid scale (11400×8000×13000µm, ratio ~14.25:10:16.25).
    const shellGeo = new THREE.SphereGeometry(1, 48, 32);
    shellGeo.applyMatrix4(new THREE.Matrix4().makeScale(570, 400, 650));
    ...
    // Stage 6 FIX 2: enlarged from 900×700 to 1500×1200 (brain shell 1140 wide at widest
    // cut — 900 was smaller than brain cross-section; 1500 comfortably exceeds it).
    const planeGeo = new THREE.PlaneGeometry(1500, 1200);
    ...
    // Stage 6 FIX 2: EdgesGeometry also updated to match enlarged PlaneGeometry 1500×1200.
    const edgesGeo  = new THREE.EdgesGeometry(new THREE.PlaneGeometry(1500, 1200));
    ...
    function animate() {
      ...
      // Stage 6 FIX 1: rotation.set arg order swapped to match buildSlicePlane Euler remapping:
      //   arg1 (Euler X) = roll  (somersault)
      //   arg2 (Euler Y) = yaw   (left-right swing)
      //   arg3 (Euler Z) = pitch (axial spin)
      ...
      if (planeEdgesRef.current) {
        // Stage 6 FIX 1: same swap applied to edges widget for visual consistency.
    ...
    function onWheel(e) {
      ...
      // Stage 6 FIX 2: clamp updated (80→300 min, 2000→6000 max).
      // min 80 allowed camera inside structure meshes (200–800 unit radii); min 300 prevents this.
      // max 2000 barely exceeded recommended initial radius 1600; max 6000 gives room to zoom out.
      orbitRef.current.radius = Math.max(300, Math.min(6000, orbitRef.current.radius));
      updateCamera();
    }
  ...
  useEffect(() => {
    ...
    for (let i = -halfCount; i <= halfCount; i++) {
      if (i === 0) continue;
      // Stage 6 FIX 2: ghost plane enlarged from 900×700 to 1500×1200 (matches plane widget).
      const ghostGeo = new THREE.PlaneGeometry(1500, 1200);
      ...
      ghost.renderOrder = 3;
      // Stage 6 FIX 1: arg order swapped — roll→X (somersault), yaw→Y, pitch→Z (axial spin).
      ghost.rotation.set(
    ...
    <>
      ...
      <div style={{
        ...
        <div style={{
          ...
          {/* ---- Left slider: TILT (somersault — Euler X) ---- */}
          {/* Stage 6 FIX 1: relabeled ROLL → TILT. This slider drives sliceState.roll which
              now maps to Euler X (somersault motion: blade tips forward/back). ROLL label was
              misleading — it implied axial spin (Euler Z), which is now the right-panel input. */}
        ...
        <div style={{
          ...
          >
            ...
            {/* Stage 5 — Pitch/roll numeric input (replaces pitch slider).
                Stage 6 FIX 1: relabeled PITCH → ROLL and description "A/P tilt" → "axial spin".
                sliceState.pitch now maps to Euler Z (axial spin / head-tilt) after the axis
                remapping in buildSlicePlane. The variable is still named `pitch` in sliceState
                for backward-compatibility with saved session JSON, but its anatomical meaning
                is now axial spin (rotation around the plane's face-normal direction). */}
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
              <span style={{ fontFamily: '"DM Mono",monospace', fontSize: 10, color: '#8A8078', letterSpacing: '0.1em' }}>ROLL</span>
              <input
              ...
              <span style={{ fontFamily: '"DM Mono",monospace', fontSize: 9, color: '#AAA098' }}>axial spin</span>
            </div>
